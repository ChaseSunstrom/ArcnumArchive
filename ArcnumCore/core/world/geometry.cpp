...
namespace arcnum_core
{
	triangle::triangle(std::vector<float> vertices, float world_position[3])
	{
		...
		//verticy 2
		vertices[3] += world_position[0];
		vertices[4] += world_position[1];
		vertices[5] += world_position[2];

		//verticy 3
		vertices[6] += world_position[0];
		vertices[7] += world_position[1];
		vertices[8] += world_position[2];
